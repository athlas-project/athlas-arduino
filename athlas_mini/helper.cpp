...
void Leg::setupLeg(unsigned int servoPin,unsigned int analogPin,unsigned int servoDir){
  this->pid.setTa(5);
  ...
  this->state = LEG_STATE_FORCE_CONTROLL;
...
void Leg::setHeight(double height){
  bool windup = false;
...
void Leg::setForce(int force){
  this->forceValueReference = force;
}

void Leg::updateControllLoop(){
  if(state == LEG_STATE_UP){
    this->setHeight(0);
  }else if(state == LEG_STATE_FORCE_CONTROLL){
...
double Leg::heightToAngle(double *height_pointer,bool *windup){
